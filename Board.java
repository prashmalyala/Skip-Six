...
public abstract class Board {
   ...
   *                    the deck
   */
  public Board(int size, String[] ranks, String[] suits, int[] pointValues, int numPlayers) {
    cards = new Card[size];
    deck = new Deck(ranks, suits, pointValues);
    player = new Player[numPlayers];
    for (int i = 0; i< numPlayers; i++){
      player[i] = new Player(deck, size);
    }    
    ...
    player[currentPlayer].dealMyCards();
    discardPileTopCard = deck.deal();
  }
  ...
  */
   ...
   *         false otherwise.
   */
  public abstract boolean isLegal(int num);
  ...
  public void drawOneCard() {
    player[currentPlayer].resize();
    cards[cards.length-1] = deck.deal();
    for (int k = 0; k < cards.length; k++) {
  ...
  public void setDiscardPileTopCard (Card playedCard) {
    discardPileTopCard = playedCard;
  }
